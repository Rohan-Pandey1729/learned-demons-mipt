...
(GRU) global memory, trained with PPO; actions are sampled without
replacement through a Plackett--Luce factorization, which gives exact
log-probabilities. The recurrent state exists precisely so that the policy
class can express time-advancing patterns such as sweeps. Training used
400 updates of 64 episodes (25{,}600 episodes) at $L = 32$, $k = 4$,
$T = 128$, with per-layer entropy-difference reward shaping.

Table~\ref{tab:learned} reports matched-budget evaluations. The CEM linear
policy improves modestly on random placement and converges to a
``fencing'' strategy: it pins measurements at a small set of fixed sites
and quarantines entanglement between them (Fig.~\ref{fig:spacetime}).
Fencing is locally sensible and globally suboptimal. The PPO policy, given
$19\times$ more episodes and a policy class that contains moving patterns,
...
credit assignment. \todo{optional: seed PPO from a sweep-like
initialization and test stability of the optimum}
