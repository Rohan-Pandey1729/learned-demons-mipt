...
model, the placement policies, and the exact record-entropy ledger.
...
wrapping left-to-right scan; (iii) an equal-coverage control that visits
...
(Sec.~\ref{sec:learned}). Entanglement entropies of stabilizer states are
computed exactly via $\mathrm{GF}(2)$ ranks~\cite{Fattal2004}, and
simulations use \texttt{stim}~\cite{AaronsonGottesman2004,Gidney2021}. We
use circuit depth $T = 4L$ and locate transitions with the tripartite
mutual information $\I_3$ of contiguous quarters, whose crossings for
different $L$ identify $p_c$~\cite{Gullans2020,Zabalo2020}. Details are
given in Appendix~\ref{app:numerics}.
...
$S \sim v_E t$, so the steady state saturates at
$S_\mathrm{max} \sim v_E/p$ for any $L \gg 1/p$. The result is an area law
at every fixed $p > 0$, that is, $p_c^{\rm sweep} = 0$ in the
thermodynamic limit. The argument predicts the single-scale form
$S = p^{-1} f(pL)$ with $f(x) \propto x$ at small $x$ and
$f \to f_\infty$ at large $x$. This form collapses the data for
...
All simulation code, trained policies, and the data behind every figure
are available at
\url{https://github.com/Rohan-Pandey1729/learned-demons-mipt}.
...
\emph{Entropies.} For a pure stabilizer state with generator matrix $G$
($n$ rows, binary symplectic columns), the entanglement entropy of a
region $A$ is $S_A = \mathrm{rank}_{\mathrm{GF}(2)}(G|_A) - |A|$
bits~\cite{Fattal2004}. The tripartite mutual information is
$\I_3(A{:}B{:}C) = S_A + S_B + S_C - S_{AB} - S_{AC} - S_{BC} + S_{ABC}$
for contiguous quarters $A, B, C, D$.
...
and a linear head on the pooled representation yields the value estimate.
Actions are $k$ sites sampled without replacement by sequential softmax
(Plackett--Luce), whose log-probability is the sum of the conditional
log-probabilities. Training: Adam with learning rate $3\times 10^{-4}$,
discount $\gamma = 0.995$, GAE $\lambda = 0.95$, clip $0.2$, entropy
bonus $0.01$, value coefficient $0.5$, 4 epochs per update, 64 episodes
per update, 400 updates. The reward is the per-layer decrease of the
half-cut entropy, plus the negative final half-cut entropy at the
terminal step. Reward shaping uses quantum-side information during
training only; the deployed policy acts on classical features alone.
